...
(
    ...
    output logic    [11:0]    led_line_order
);
// Read operations performed on the Avalon-MM Slave interface
always_comb begin
    if (avs_s0_read) begin
        case (avs_s0_address)
            1'b0    : avs_s0_readdata = {20'b0, led_line_order};
...
always_ff @ (posedge clk) begin
    if (reset) begin
        ...
        case (avs_s0_address)
            1'b0    : led_line_order <= avs_s0_writedata;
            default : led_line_order <= led_line_order;
        endcase
    end
end
